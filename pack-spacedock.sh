...
cat GameData/$TARGETDIR/KeepItStraight.cfg | sed 's/ckan_ready = False/ckan_ready = True/g' > $pwd/bin/GameData/$TARGETDIR/KeepItStraight.cfg
cp ${pwd}/bin/ReleaseCkan/Scale.dll $pwd/bin/GameData/$TARGETBINDIR
pushd $pwd/bin
zip -u $FILE GameData/WhoAmI/Plugins/WhoAmI.dll
#zip -u $FILE GameData/WhoAmI/KeepItStraight.cfg
popd
